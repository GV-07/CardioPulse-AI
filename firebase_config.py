...
def save_user_to_firebase(username, fullname, password, provider="Manual"):
    """
    Records login identity profiles (Username, Password, Google Accounts) 
    safely into the 'users' cloud collection.
    """
    try:
...
def log_patient_report(operator_username, patient_name, risk_score, patient_metrics_dict):
    """
    Generates an explicit document record matching the logged operator,
    target patient name, calculated risk percentage, and full clinical metrics list.
    """
    try:
...
def log_user_login(username, fullname):
    ...
    try:
        ...
        session_ref.set({
            ...
            "logout_time": "Active Session"  # Acts as a live placeholder string state
        })
        return session_ref.id
    except Exception as e:
        print(f"Error writing login session timestamp to ledger: {e}")
        return None
...
def log_user_logout(session_id):
    ...
    try:
        ...
        print(f"Error updating logout timestamp to ledger: {e}")
        return False
    
def get_operator_reports(operator_username):
    """
    Queries Firestore to fetch all historical patient diagnostic reports
    generated by the currently logged-in system operator.
    """
    try:
        reports_list = []
        # Queries the database for records matching the operator's identity string
        docs = db.collection("patient_reports").where("operator_username", "==", operator_username).stream()
        
        for doc in docs:
            data = doc.to_dict()
            # Extract clinical sub-dictionary fields cleanly
            ...
            reports_list.append(report_row)
        return reports_list
    except Exception as e:
        print(f"Error querying patient reports database matrix: {e}")
        return []